...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="The iteration that letting discriminator into the training process",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            ' "constant", "constant_with_warmup", "multisteplr"]'
        ),
    )
    parser.add_argument(
        "--discr_lr_scheduler",
...
def main():
    ...
    train_transforms = A.Compose([
        ...
        A.RandomCrop(args.resolution, args.resolution),
        A.HorizontalFlip(p=0.5),
        # TODO: add more data augmentation technique
        # TODO: use l1 loss + perceptual loss rather than l2 loss + perceptual loss
    ...
    # using multi-step lr following Qi Chen.
    milestones = [40_000, 60_000, 80_000]
    scale_factor = accelerator.num_processes // (2 * args.gradient_accumulation_steps)   # correct num for scheduler
    real_milestones = list(map(lambda x: x * scale_factor, milestones))
    lr_scheduler = MultiStepLR(optimizer=optimizer, milestones=real_milestones, gamma=0.1, verbose=True)
    discr_lr_scheduler = MultiStepLR(optimizer=discr_optimizer, milestones=real_milestones, gamma=0.1, verbose=True)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            # print("input:", pixel_values.min(), pixel_values.max())
            ...
            mask_disc = int(global_step > args.discriminator_iter_start)    # NOTE: before adding discriminator to training, mask the loss out!

            if generator_step:
                with accelerator.accumulate(model):
                    # reconstruction loss. Pixel level differences between input vs output
                    if args.vae_loss == "l2":
                        loss = F.mse_loss(pixel_values, fmap)   # in the original paper
                    else:
                        loss = F.l1_loss(pixel_values, fmap)    # in the official implementation
                    ...
                    if global_step > args.discriminator_iter_start: 
                        ...
                        loss += adaptive_weight * gen_loss * mask_disc  
                    else:
                        adaptive_weight = torch.tensor(0., requires_grad=False)
                        gen_loss = torch.tensor(0., requires_grad=False)
                    
                    # DEBUG:
                    # print(f"[{global_step}] output: [{fmap.min().item():.2f}, {fmap.max().item():.2f}], recon_loss: {loss.item():.3f}, perceptual_loss: {perceptual_loss.item():.3f}, commit_loss: {commit_loss.item():.3f}, gan_loss: {gen_loss.item():.3f}, adaptive_weight: {adaptive_weight.item():.3f}")
            ...
            # haven't put discriminator in the training process yet.
            elif not generator_step and global_step <= args.discriminator_iter_start:
                loss = torch.tensor(0., requires_grad=False).to(device=accelerator.device)    # prevent logging error
                avg_discr_loss = accelerator.gather(loss.repeat(args.train_batch_size)).mean()

            # put discriminator in the training process!!!
            else:
